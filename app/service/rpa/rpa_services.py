...
def handle_webhook_request(request: MeliusWebhookRequest, db_session: DBSession):
    ...
    message_name = f"retorno_rpa_{rpa_event_logs[0].event_data['tipoTarefaRpa']}"
    camunda_request = CamundaRequest(
        message_name=message_name,
        process_variables={
            message_name: {
                "value": request.model_dump(include=["status_tarefa_rpa", "mensagem_retorno", "arquivos_gerados"]),
            },
        },
